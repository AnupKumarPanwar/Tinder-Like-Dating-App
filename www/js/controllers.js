    ...
    document.addEventListener("deviceready", function onDeviceReady(w) {
      ...
      // window.plugins.OneSignal.setLogLevel({logLevel: 4, visualLevel: 4});
            ...
            var notificationOpenedCallback = function(jsonData) {
              ...
              // if (jsonData.notification.payload.additionalData.status=="like") {
              //   // alert('Awesome! You won the Challenge :) Your opponent scored just '+jsonData.notification.payload.additionalData.score+' points.');
                
                  $scope.set_current_card(jsonData.notification.payload.additionalData);
                    ...
                    $scope.showProfile(jsonData.notification.payload.additionalData);
            ...
            window.plugins.OneSignal
              .startInit("91661b97-c784-4623-ba85-04254f6b7ea8")
...
.then(function(api)
{
  ...
  // AdMob.showRewardVideoAd();
    ...
    .then(function(data)
    {
      $scope.cardTypes=data.data.data;
      $scope.last=data.data.data[22].last;
      $rootScope.last=$scope.last;
      console.log($scope.last);
...
var leftSwipeCount=0;
  ...
  $scope.set_current_card=function(card)
  {
    console.log(card.instagram_username);
    if (card.instagram_username=="") {
      var image=[{images:{standard_resolution:{url:card.photo_url}}}];
              $scope.userDp=card.photo_url;
              $scope.instagram_images=image;
              console.log($scope.instagram_images);
    }
    else{

    var instagram_username=card.instagram_username;
    instagram_username=instagram_username.replace(" ","_");
    if (instagram_username[0]=='_') {
      instagram_username=instagram_username.substr(1);
    }
    
    var instagramUrl="https://nearcabs.000webhostapp.com/thinder/instagram.php?username="+instagram_username;
    ...
    .then(function(data)
    {
      if (data.data.items.length==0)
       ...
       {
        $scope.userDp=data.data.items[0].user.profile_picture;
        $scope.instagram_images=data.data.items;
        console.log($scope.instagram_images);
       }
    })
  }
...
$scope.openSnapchat=function()
{
    AdMob.showInterstitial();
    console.log($scope.current_card.snapchat_username);
  // startApp.set({ /* params */
  //  "action": "ACTION_SEND",
  //  "package": "com.snapchat.android",
  //  "type": "text/plain",
  //  "uri":"http://snapchat.com/add/"+$scope.current_card.snapchat_username
  // }, {
  // }).start();
  window.open('https://snapchat.com/add/'+$scope.current_card.snapchat_username , '_system' , 'location=no, clearcache=no, clearsessioncache=no');
